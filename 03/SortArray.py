#3.1 Implementa BubbleSort bidireccional
...
class Array(object):
   ...
   def bubbleSortBi(self):
      first=0
      for last in range(self.nElementos-1,first, -1):
         for first in range (0, last, 1):
            for inner in range(first, last):
               if self.__a[inner] > self.__a[inner+1]:
                  print(self.__a, self.__a[inner], "R")                  
                  self.swap(inner, inner+1)
                  
            for inner in range(last-1,first, -1):               
               if self.__a[inner] < self.__a[inner-1]:
                  print(self.__a, self.__a[inner], "L")
                  self.swap(inner, inner-1)
